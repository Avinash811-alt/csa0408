...
def preemptive_priority_scheduling(processes, burst_time, arrival_time, priority):
    ...
    while complete != n:
        # Select process with highest priority among available processes
        idx = -1
        highest_priority = -1
        for i in range(n):
        ...
        if idx != -1:
            ...
            time += 1  # If no process has arrived, increment time
...
burst_time = [7, 4, 1, 4]
priority = [2, 3, 5, 4]  # Higher number = higher priority
